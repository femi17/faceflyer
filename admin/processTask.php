...
if($post == $rs['count']){
header('Location: add-task?upgrade=1');
exit;
}
}
...
unlink($small_thumbnail_path.fetchData('tasks', $id, 'id', 'banner'));

$thumb1 = move_uploaded_file($image, $small_thumbnail);

if($id){

mysqli_query($connect, "update tasks set banner = '$banner' where id = '$id'");
...
if($id){
mysqli_query($connect, "update tasks set name = '$name', category = '$category', end_date = '$end', description = '$desc', coin = '$coin', status = 'approved', date = '$date' where id = '$id'");
}else{
mysqli_query($connect, "insert into tasks set store = '$email', name = '$name', category = '$category', end_date = '$end', description = '$desc', coin = '$coin', status = 'approved', date = '$date', banner = '$banner'");
}

header('Location: add-task?add=1');
exit;
}else{

header('Location: add-task?captcha=1');
exit;
