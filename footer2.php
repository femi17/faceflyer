<section class="footer section-padding">
<div class="container">
<div class="row py-lg-4">
<div class="col-xl-2 col-sm-6">
<div class="footer-brand">
<img class="img-fluid" src="../assets/images/logo.png">
</div>
</div>
<div class="col-xl-2 col-sm-6">
<h6 class="text-gray-900">Faceflyer</h6>
<ul>
<li><a href="../about-us">About Us</a></li>
<li><a href="../how-it-works">How it Works</a></li>
<li><a href="../partner-with-us">Faceflyer Merchant</a></li>
</ul>
</div>
<div class="col-xl-2 col-sm-6">
<h6 class="text-gray-900">Ways to Earn</h6>
<ul>
<li><a href="../tasks">Tasks</a></li>
<li><a href="../deals">Deals</a></li>
<li><a href="../cashback">Cashback</a></li>
</ul>
</div>
<div class="col-xl-2 col-sm-6">
<h6 class="text-gray-900">CONNECT</h6>
<ul>
<li><a href="../contact-us">Contact Us</a></li>
<li><a href="../contact-us">Advertise With Us</a></li>
<li><a href="../support">Support</a></li>
</ul>
</div>
<div class="col-xl-4 col-sm-6">
<h6 class="text-gray-900">Subscribe to our Newsletter</h6>
<p id="subMsg2"></p>
<form class="form-inline newsletter-form mb-1">
<input type="text" class="form-control mr-sm-2" id="subEmail2" placeholder="Enter your email">
<button type="button" id="submitEmail2" class="btn btn-primary">Subscribe</button>
</form>
<small><a href="javascript:;">Register now to get updates on <span class="text-info">Tasks, Deals and
Cashback</span></a></small>
<div class="app mt-4 pt-2">
<h6 class="text-gray-900">COMING SOON</h6>
<a href="javascript:;">
<img class="img-fluid" src="../assets/images/google.png">
</a>
<a href="javascript:;">
<img class="img-fluid" src="../assets/images/apple.png">
</a>
</div>
</div>
</div>
</div>
</section>
<section class="py-4 bg-white osahan-copyright">
<div class="container">
<div class="row">
<div class="col-xl-12">
<div class="d-flex align-items-center justify-content-between">
<p class="m-0 text-secondary">Copyright © faceflyer <?php echo date('Y'); ?></p>
<p class="m-0">
<a href="../privacy-policy" class="text-secondary">Privacy Policy</a>
&nbsp; · &nbsp;
<a href="../terms" class="text-secondary">Terms &amp; Conditions</a>
</p>
<ul class="list-inline mb-0">
<li class="list-inline-item">
<a class="border rounded p-2" href="https://www.facebook.com/Faceflyer-114018070424187/" target="_blank">
<span class="icofont-facebook"></span>
</a>
</li>
<li class="list-inline-item">
<a class="border rounded p-2" href="https://www.instagram.com/faceflyerng/" target="_blank">
<span class="icofont-instagram"></span>
</a>
</li>
<li class="list-inline-item">
<a class="border rounded p-2" href="https://twitter.com/faceflyer1" target="_blank">
<span class="icofont-twitter"></span>
</a>
</li>
</ul>
</div>
</div>
</div>
</div>
</section>
