...
$id = $_GET['i'];
$qs = mysqli_query($connect, "select * from tasks where store = '{$_SESSION['faceMerchant']}' && id = '$id'");
...
<title>Add task - Merchant - Faceflyer</title>
<!-- Favicon Icon -->
...
<h5 class="mb-0 text-gray-900">Add a task</h5>
</div>
...
<div class="col-md-12">
<?php if($_GET['add']){ ?>
<p style="color:green"><strong>Task uploaded successfully</strong></p>
<?php } ?>
<?php if($_GET['upgrade']){ ?>
...
<form class="js-validate" action="processTask" method="post" enctype="multipart/form-data" id="task-form">
<div class="row">
<!-- Input -->
...
<div class="form-group">
<select class="form-control specify" name="category" required>
<?php if($rs['category']){ ?>
...
grecaptcha.execute('6LcNS7AZAAAAAHeOWW2bDxQCH3nSYF13kaYUa0Sy', {action: 'upload_task'}).then(function(token) {
$('#task-form').prepend('<input type="hidden" name="token" value="' + token + '">');
$('#task-form').prepend('<input type="hidden" name="action" value="upload_task">');
$('#task-form').unbind('submit').submit();
});;
});
});
</script>
...
placeholder: 'Describe how a facer will complete the task. Put action point on a new line [shift+enter] to go to new line',
height: 150
});
</script>
</body>
